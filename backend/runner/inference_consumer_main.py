import pika, sys, os
from mq_route import inference_consumer # rabbitmq 和 训练代码之间的接口函数
import torch
...
def main():
    ...
    channel.queue_bind(exchange='dl_task', queue=queue_name, routing_key='inference')

    print(' [*] Waiting for messages. To exit press CTRL+C')
    
    # 每个进程只接收一个训练任务！！！ 设置 quality-of-service
    channel.basic_qos(prefetch_count=1) # quality of service
    channel.basic_consume(queue = queue_name, on_message_callback = inference_consumer, auto_ack = True)
